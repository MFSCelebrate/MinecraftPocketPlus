...
void Minecraft::update() {
    ...
    // 注意：这里不再有 TIMER_PUSH("tick")，tick 函数内部自己包裹
...
#endif
    //
    // Ongoing level generation in a (perhaps) different thread.
    //
    if (isGeneratingLevel) {
        TIMER_POP();   // ★ 提前返回前弹出 tick
        return;
