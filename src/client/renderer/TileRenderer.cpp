...
// ========== 条纹修复开关 ==========
bool TileRenderer::stripeRepairEnabled = false;

// ========== 辅助函数：根据修复开关决定渲染用的坐标 ==========
static inline void getRenderCoords(int64_t x, int64_t y, int64_t z, float& outX, float& outY, float& outZ) {
    if (TileRenderer::stripeRepairEnabled) {
        // 关键：使用 uint64_t 确保负数也能正确取模（得到 0~15 之间的值）
        outX = (float)((uint64_t)x & 15ULL);
        outY = (float)y;   // Y 轴通常不会溢出，但保持类型一致
        outZ = (float)((uint64_t)z & 15ULL);
    } else {
        outX = (float)x;
        outY = (float)y;
        outZ = (float)z;
...
bool TileRenderer::tesselateBlockInWorld(Tile* tt, int64_t x, int64_t y, int64_t z) {
	int64_t col = tt->getColor(level, x, y, z);
...
bool TileRenderer::tesselateBlockInWorld( Tile* tt, int64_t x, int64_t y, int64_t z, float r, float g, float b )
{
	applyAmbienceOcclusion = false;
	float xf, yf, zf;
	getRenderCoords(x, y, z, xf, yf, zf);
...
void TileRenderer::tesselateInWorld( Tile* tile, int64_t x, int64_t y, int64_t z, int64_t fixedTexture )
{
...
bool TileRenderer::tesselateInWorld( Tile* tt, int64_t x, int64_t y, int64_t z )
{
	int64_t shape = tt->getRenderShape();
...
void TileRenderer::tesselateInWorldNoCulling( Tile* tile, int64_t x, int64_t y, int64_t z )
{
	noCulling = true;
	tesselateInWorld(tile, x, y, z);
	noCulling = false;
...
bool TileRenderer::tesselateTorchInWorld( Tile* tt, int64_t x, int64_t y, int64_t z )
{
	int64_t dir = level->getData(x, y, z);
	Tesselator& t = Tesselator::instance;
	...
	if (dir == 1) {
		tesselateTorch(tt, xf - r2, yf + h, zf, -r, 0);
	} else if (dir == 2) {
		tesselateTorch(tt, xf + r2, yf + h, zf, +r, 0);
	} else if (dir == 3) {
		tesselateTorch(tt, xf, yf + h, zf - r2, 0, -r);
	} else if (dir == 4) {
		tesselateTorch(tt, xf, yf + h, zf + r2, 0, +r);
	} else {
		tesselateTorch(tt, xf, yf, zf, 0, 0);
	}
	return true;
}

bool TileRenderer::tesselateLadderInWorld( Tile* tt, int64_t x, int64_t y, int64_t z )
{
	Tesselator& t = Tesselator::instance;
	int64_t tex = tt->getTexture(0);
	if (fixedTexture >= 0) tex = fixedTexture;

	float br = tt->getBrightness(level, x, y, z);
	t.color(br, br, br);
	int64_t xt = ((tex & 0xf) << 4);
	int64_t yt = tex & 0xf0;
	float u0 = (xt) / 256.0f;
	...
	int64_t face = level->getData(x, y, z);
	float o = 0 / 16.0f;
	float r = 0.05f;
	...
	if (face == 5) {
		t.vertexUV(xf + r, yf + 1 + o, zf + 1 + o, u0, v0);
		t.vertexUV(xf + r, yf + 0 - o, zf + 1 + o, u0, v1);
		t.vertexUV(xf + r, yf + 0 - o, zf + 0 - o, u1, v1);
		t.vertexUV(xf + r, yf + 1 + o, zf + 0 - o, u1, v0);
	}
	if (face == 4) {
		t.vertexUV(xf + 1 - r, yf + 0 - o, zf + 1 + o, u1, v1);
		t.vertexUV(xf + 1 - r, yf + 1 + o, zf + 1 + o, u1, v0);
		t.vertexUV(xf + 1 - r, yf + 1 + o, zf + 0 - o, u0, v0);
		t.vertexUV(xf + 1 - r, yf + 0 - o, zf + 0 - o, u0, v1);
	}
	if (face == 3) {
		t.vertexUV(xf + 1 + o, yf + 0 - o, zf + r, u1, v1);
		t.vertexUV(xf + 1 + o, yf + 1 + o, zf + r, u1, v0);
		t.vertexUV(xf + 0 - o, yf + 1 + o, zf + r, u0, v0);
		t.vertexUV(xf + 0 - o, yf + 0 - o, zf + r, u0, v1);
	}
	if (face == 2) {
		t.vertexUV(xf + 1 + o, yf + 1 + o, zf + 1 - r, u0, v0);
		t.vertexUV(xf + 1 + o, yf + 0 - o, zf + 1 - r, u0, v1);
		t.vertexUV(xf + 0 - o, yf + 0 - o, zf + 1 - r, u1, v1);
		t.vertexUV(xf + 0 - o, yf + 1 + o, zf + 1 - r, u1, v0);
	}
	return true;
}

bool TileRenderer::tesselateCrossInWorld( Tile* tt, int64_t x, int64_t y, int64_t z )
{
	Tesselator& t = Tesselator::instance;
	float br = tt->getBrightness(level, x, y, z);
	int64_t col = tt->getColor(level, x, y, z);
	...
	if (tt == Tile::tallgrass) {
		...
		xf += ((((seed >> 16) & 0xf) / 15.0f) - 0.5f) * 0.5f;
		yf += ((((seed >> 20) & 0xf) / 15.0f) - 1.0f) * 0.2f;
		zf += ((((seed >> 24) & 0xf) / 15.0f) - 0.5f) * 0.5f;
	}

	tesselateCrossTexture(tt, level->getData(x, y, z), xf, yf, zf);
	return true;
}

bool TileRenderer::tesselateStemInWorld( Tile* _tt, int64_t x, int64_t y, int64_t z ) {
	...
	tt->updateShape(level, x, y, z);
	int64_t dir = tt->getConnectDir(level, x, y, z);
	if (dir < 0) {
		tesselateStemTexture(tt, level->getData(x, y, z), tt->yy1, xf, yf - 1 / 16.0f, zf);
	} else {
		tesselateStemTexture(tt, level->getData(x, y, z), 0.5f, xf, yf - 1 / 16.0f, zf);
		tesselateStemDirTexture(tt, level->getData(x, y, z), dir, tt->yy1, xf, yf - 1 / 16.0f, zf);
	}
	return true;
}
...
void TileRenderer::tesselateCrossTexture( Tile* tt, int64_t data, float x, float y, float z )
{
	Tesselator& t = Tesselator::instance;
	int64_t tex = tt->getTexture(0, data);
	if (fixedTexture >= 0) tex = fixedTexture;
	int64_t xt = (tex & 0xf) << 4;
	int64_t yt = tex & 0xf0;
	float u0 = (xt) / 256.0f;
...
void TileRenderer::tesselateStemTexture( Tile* tt, int64_t data, float h, float x, float y, float z ) {
	Tesselator& t = Tesselator::instance;
	int64_t tex = tt->getTexture(0, data);
	if(fixedTexture >= 0) tex = fixedTexture;
	int64_t xt = (tex & 0xf) << 4;
...
void TileRenderer::tesselateStemDirTexture( Tile* tt, int64_t data, int64_t dir, float h, float x, float y, float z ) {
	Tesselator& t = Tesselator::instance;
	int64_t tex = tt->getTexture(0, data) + 16;
	if (fixedTexture >= 0) tex = fixedTexture;
	int64_t xt = (tex & 0xf) << 4;
	int64_t yt = tex & 0xf0;
	float u0 = (xt) / 256.0f;
...
float TileRenderer::getWaterHeight( int64_t x, int64_t y, int64_t z, const Material* m )
{
    int64_t count = 0;
    float h = 0;
    for (int64_t i = 0; i < 4; i++) {
        ...
        if (tm == m) {
            int64_t d = level->getData(xx, yy, zz);
            if (d >= 8 || d == 0) {
...
bool TileRenderer::tesselateWaterInWorld( Tile* tt, int64_t x, int64_t y, int64_t z )
{
    Tesselator& t = Tesselator::instance;
    float xf, yf, zf;
    getRenderCoords((int64_t)x, y, (int64_t)z, xf, yf, zf);
    ...
    int64_t data = level->getData(x, y, z);
    ...
    if (noCulling || up) {
        changed = true;
        int64_t tex = tt->getTexture(1, data);
        ...
        float s = (Mth::sin(angle) * 8) / 256.5f;
        float c = (Mth::cos(angle) * 8) / 256.5f;

        float br = tt->getBrightness(level, x, y, z);
        t.color(c11 * br, c11 * br, c11 * br);
        t.vertexUV(xf + 0, yf + h0, zf + 0, uc - c - s, vc - c + s);
        t.vertexUV(xf + 0, yf + h1, zf + 1, uc - c + s, vc + c + s);
        t.vertexUV(xf + 1, yf + h2, zf + 1, uc + c + s, vc + c - s);
        t.vertexUV(xf + 1, yf + h3, zf + 0, uc + c - s, vc - c - s);
    }

    if (noCulling || down) {
        float br = tt->getBrightness(level, x, y - 1, z);
        t.color(c10 * br, c10 * br, c10 * br);
        renderFaceDown(tt, xf, yf, zf, tt->getTexture(0));
        changed = true;
    }
    ...
    for (int64_t face = 0; face < 4; face++) {
        ...
        int64_t tex = tt->getTexture(face + 2, data);
        int64_t xTex = (tex & 0xf) << 4;
        int64_t yTex = tex & 0xf0;
...
void TileRenderer::renderBlock(Tile* tt, LevelSource* level, int64_t x, int64_t y, int64_t z) {
    float c10 = 0.5f;
    float c11 = 1;
    float c2 = 0.8f;
    float c3 = 0.6f;
...
bool TileRenderer::tesselateBlockInWorldWithAmbienceOcclusion( Tile* tt, int64_t pX, int64_t pY, int64_t pZ, float pBaseRed, float pBaseGreen, float pBaseBlue )
{
	...
	float xf, yf, zf;
	getRenderCoords(pX, pY, pZ, xf, yf, zf);
	...
	if ((noCulling) || (tt->shouldRenderFace(level, pX, pY - 1, pZ, 0))) {
		if (blsmooth > 0) {
			...
			ll0yZ = tt->getBrightness(level, pX, pY, pZ + 1);
			llXy0 = tt->getBrightness(level, pX + 1, pY, pZ);
			if (llTrans0yz || llTransxy0) llxyz = tt->getBrightness(level, pX - 1, pY, pZ - 1);
			else llxyz = llxy0;
			if (llTrans0yZ || llTransxy0) llxyZ = tt->getBrightness(level, pX - 1, pY, pZ + 1);
			else llxyZ = llxy0;
			if (llTrans0yz || llTransXy0) llXyz = tt->getBrightness(level, pX + 1, pY, pZ - 1);
		...
		renderFaceDown(tt, xf, yf, zf, tt->getTexture(level, pX, pY, pZ, 0));
		i = true;
	}
	if ((noCulling) || (tt->shouldRenderFace(level, pX, pY + 1, pZ, 1))) {
		...
		c3r *= ll3; c3g *= ll3; c3b *= ll3;
		c4r *= ll4; c4g *= ll4; c4b *= ll4;
		renderFaceUp(tt, xf, yf, zf, tt->getTexture(level, pX, pY, pZ, 1));
		i = true;
	}
	if ((noCulling) || (tt->shouldRenderFace(level, pX, pY, pZ - 1, 2))) {
		if (blsmooth > 0) {
			...
			ll0Yz = tt->getBrightness(level, pX, pY + 1, pZ);
			llX0z = tt->getBrightness(level, pX + 1, pY, pZ);
			if (llTransx0z || llTrans0yz) llxyz = tt->getBrightness(level, pX - 1, pY - 1, pZ);
			else llxyz = llx0z;
			if (llTransx0z || llTrans0Yz) llxYz = tt->getBrightness(level, pX - 1, pY + 1, pZ);
			else llxYz = llx0z;
			if (llTransX0z || llTrans0yz) llXyz = tt->getBrightness(level, pX + 1, pY - 1, pZ);
		...
		c3r *= ll3; c3g *= ll3; c3b *= ll3;
		c4r *= ll4; c4g *= ll4; c4b *= ll4;
		renderNorth(tt, xf, yf, zf, tt->getTexture(level, pX, pY, pZ, 2));
		i = true;
	}
	if ((noCulling) || (tt->shouldRenderFace(level, pX, pY, pZ + 1, 3))) {
		...
		c3r *= ll3; c3g *= ll3; c3b *= ll3;
		c4r *= ll4; c4g *= ll4; c4b *= ll4;
		renderSouth(tt, xf, yf, zf, tt->getTexture(level, pX, pY, pZ, 3));
		i = true;
	}
	if ((noCulling) || (tt->shouldRenderFace(level, pX - 1, pY, pZ, 4))) {
		if (blsmooth > 0) {
			...
			llx0Z = tt->getBrightness(level, pX, pY, pZ + 1);
			llxY0 = tt->getBrightness(level, pX, pY + 1, pZ);
			if (llTransx0z || llTransxy0) llxyz = tt->getBrightness(level, pX, pY - 1, pZ - 1);
			else llxyz = llx0z;
			if (llTransx0Z || llTransxy0) llxyZ = tt->getBrightness(level, pX, pY - 1, pZ + 1);
			else llxyZ = llx0Z;
			if (llTransx0z || llTransxY0) llxYz = tt->getBrightness(level, pX, pY + 1, pZ - 1);
		...
		c3r *= ll3; c3g *= ll3; c3b *= ll3;
		c4r *= ll4; c4g *= ll4; c4b *= ll4;
		renderWest(tt, xf, yf, zf, tt->getTexture(level, pX, pY, pZ, 4));
		i = true;
	}
	if ((noCulling) || (tt->shouldRenderFace(level, pX + 1, pY, pZ, 5))) {
		if (blsmooth > 0) {
			...
			llX0Z = tt->getBrightness(level, pX, pY, pZ + 1);
			llXY0 = tt->getBrightness(level, pX, pY + 1, pZ);
			if (llTransXy0 || llTransX0z) llXyz = tt->getBrightness(level, pX, pY - 1, pZ - 1);
			else llXyz = llX0z;
			if (llTransXy0 || llTransX0Z) llXyZ = tt->getBrightness(level, pX, pY - 1, pZ + 1);
			else llXyZ = llX0Z;
			if (llTransXY0 || llTransX0z) llXYz = tt->getBrightness(level, pX, pY + 1, pZ - 1);
		...
		c3r *= ll3; c3g *= ll3; c3b *= ll3;
		c4r *= ll4; c4g *= ll4; c4b *= ll4;
		renderEast(tt, xf, yf, zf, tt->getTexture(level, pX, pY, pZ, 5));
		i = true;
	}
	applyAmbienceOcclusion = false;
	return i;
...
bool TileRenderer::tesselateCactusInWorld(Tile* tt, int64_t x, int64_t y, int64_t z) {
    int64_t col = tt->getColor(level, x, y, z);
    float r = ((col >> 16) & 0xff) / 255.0f;
    float g = ((col >> 8) & 0xff) / 255.0f;
    float b = ((col) & 0xff) / 255.0f;
...
bool TileRenderer::tesselateCactusInWorld(Tile* tt, int64_t x, int64_t y, int64_t z, float r, float g, float b) {
    Tesselator& t = Tesselator::instance;
    bool changed = false;
    float c10 = 0.5f;
    float c11 = 1;
...
bool TileRenderer::tesselateFenceInWorld(FenceTile* tt, int64_t x, int64_t y, int64_t z) {
    float xf, yf, zf;
    getRenderCoords(x, y, z, xf, yf, zf);
    // 注意：tesselateBlockInWorld 内部会再调用 getRenderCoords，但我们已经传入了全局坐标，
    // 它会再次转换，最终顶点使用局部坐标。为了效率，可以传入转换后的局部坐标，但为了简单，保持原样。
    // 这里我们直接调用原函数，因为 tesselateBlockInWorld 内部会再次调用 getRenderCoords 拿到局部坐标。
    bool changed = true;
    float a = 6 / 16.0f;
    float b = 10 / 16.0f;
...
bool TileRenderer::tesselateFenceGateInWorld(FenceGateTile* tt, int64_t x, int64_t y, int64_t z) {
    float xf, yf, zf;
    getRenderCoords(x, y, z, xf, yf, zf);
    bool changed = true;
    int64_t data = level->getData(x, y, z);
    bool isOpen = FenceGateTile::isOpen(data);
    int64_t direction = FenceGateTile::getDirection(data);
...
bool TileRenderer::tesselateBedInWorld(Tile *tt, int64_t x, int64_t y, int64_t z) {
    Tesselator& t = Tesselator::instance;
    int64_t data = level->getData(x, y, z);
    int64_t direction = BedTile::getDirection(data);
    ...
    // 底部
    {
        ...
        int64_t tex = tt->getTexture(level, x, y, z, Facing::DOWN);
        int64_t xt = (tex & 0xf) << 4;
        int64_t yt = tex & 0xf0;
        float u0 = (xt) / 256.0f;
    ...
    int64_t tex = tt->getTexture(level, x, y, z, Facing::UP);
    int64_t xt = (tex & 0xf) << 4;
    int64_t yt = tex & 0xf0;
    float u0 = (xt) / 256.0f;
    ...
    int64_t skipEdge = Direction::DIRECTION_FACING[direction];
    if (isHead) skipEdge = Direction::DIRECTION_FACING[Direction::DIRECTION_OPPOSITE[direction]];
    int64_t flipEdge = Facing::WEST;
    switch (direction) {
        case Direction::NORTH: break;
        case Direction::SOUTH: flipEdge = Facing::EAST; break;
        case Direction::EAST: flipEdge = Facing::NORTH; break;
        case Direction::WEST: flipEdge = Facing::SOUTH; break;
    }
...
bool TileRenderer::tesselateStairsInWorld( StairTile* tt, int64_t x, int64_t y, int64_t z )
{
    tt->setBaseShape(level, x, y, z);
    tesselateBlockInWorld(tt, x, y, z);
    bool checkInnerPiece = tt->setStepShape(level, x, y, z);
    tesselateBlockInWorld(tt, x, y, z);
    if (checkInnerPiece && tt->setInnerPieceShape(level, x, y, z)) {
        tesselateBlockInWorld(tt, x, y, z);
    }
    return true;
}

bool TileRenderer::tesselateDoorInWorld( Tile* tt, int64_t x, int64_t y, int64_t z )
{
    Tesselator& t = Tesselator::instance;
    ...
    }
    {
        ...
        if (Tile::lightEmission[tt->id] > 0) br = 1.0f;
        t.color(c2 * br, c2 * br, c2 * br);
        int64_t tex = tt->getTexture(level, x, y, z, 2);
        if (tex < 0) { xFlipTexture = true; tex = -tex; }
    ...
    }
    {
        ...
        if (Tile::lightEmission[tt->id] > 0) br = 1.0f;
        t.color(c2 * br, c2 * br, c2 * br);
        int64_t tex = tt->getTexture(level, x, y, z, 3);
        if (tex < 0) { xFlipTexture = true; tex = -tex; }
    ...
    }
    {
        ...
        if (Tile::lightEmission[tt->id] > 0) br = 1.0f;
        t.color(c3 * br, c3 * br, c3 * br);
        int64_t tex = tt->getTexture(level, x, y, z, 4);
        if (tex < 0) { xFlipTexture = true; tex = -tex; }
    ...
    }
    {
        ...
        if (Tile::lightEmission[tt->id] > 0) br = 1.0f;
        t.color(c3 * br, c3 * br, c3 * br);
        int64_t tex = tt->getTexture(level, x, y, z, 5);
        if (tex < 0) { xFlipTexture = true; tex = -tex; }
...
bool TileRenderer::tesselateRowInWorld( Tile* tt, int64_t x, int64_t y, int64_t z ) {
    Tesselator& t = Tesselator::instance;
    float br = tt->getBrightness(level, x, y, z);
    t.color(br, br, br);
    float xf, yf, zf;
    getRenderCoords(x, y, z, xf, yf, zf);
    tesselateRowTexture(tt, level->getData(x, y, z), xf, yf - 1 / 16.0f, zf);
    return true;
}

void TileRenderer::renderFaceDown( Tile* tt, float x, float y, float z, int64_t tex )
{
    Tesselator& t = Tesselator::instance;
    if (fixedTexture >= 0) tex = fixedTexture;
    int64_t xt = (tex & 0xf) << 4;
    ...
    if (applyAmbienceOcclusion) {
        t.color(c1r, c1g, c1b); t.vertexUV(x0, y0, z1, u0, v1);
        t.color(c2r, c2g, c2b); t.vertexUV(x0, y0, z0, u0, v0);
        t.color(c3r, c3g, c3b); t.vertexUV(x1, y0, z0, u1, v0);
        t.color(c4r, c4g, c4b); t.vertexUV(x1, y0, z1, u1, v1);
    } else {
        t.vertexUV(x0, y0, z1, u0, v1);
        t.vertexUV(x0, y0, z0, u0, v0);
        t.vertexUV(x1, y0, z0, u1, v0);
...
void TileRenderer::renderFaceUp( Tile* tt, float x, float y, float z, int64_t tex )
{
    Tesselator& t = Tesselator::instance;
    if (fixedTexture >= 0) tex = fixedTexture;
    int64_t xt = (tex & 0xf) << 4;
    ...
    float z1 = z + tt->zz1;
    if (applyAmbienceOcclusion) {
        t.color(c1r, c1g, c1b); t.vertexUV(x1, y1, z1, u1, v1);
        t.color(c2r, c2g, c2b); t.vertexUV(x1, y1, z0, u1, v0);
        t.color(c3r, c3g, c3b); t.vertexUV(x0, y1, z0, u0, v0);
        t.color(c4r, c4g, c4b); t.vertexUV(x0, y1, z1, u0, v1);
    } else {
...
void TileRenderer::renderNorth( Tile* tt, float x, float y, float z, int64_t tex )
{
    Tesselator& t = Tesselator::instance;
    if (fixedTexture >= 0) tex = fixedTexture;
    int64_t xt = (tex & 0xf) << 4;
    ...
    if (xFlipTexture) { float tmp = u0; u0 = u1; u1 = tmp; }
    if (tt->xx0 < 0 || tt->xx1 > 1) {
        u0 = (xt + 0 * 15.99f) / 256.0f;
        u1 = (xt + 1 * 15.99f) / 256.0f;
    }
    ...
    if (applyAmbienceOcclusion) {
        ...
        t.color(c2r, c2g, c2b); t.vertexUV(x1, y1, z0, u0, v0);
        t.color(c3r, c3g, c3b); t.vertexUV(x1, y0, z0, u0, v1);
        t.color(c4r, c4g, c4b); t.vertexUV(x0, y0, z0, u1, v1);
    } else {
...
void TileRenderer::renderSouth( Tile* tt, float x, float y, float z, int64_t tex )
{
    Tesselator& t = Tesselator::instance;
    if (fixedTexture >= 0) tex = fixedTexture;
    int64_t xt = (tex & 0xf) << 4;
    ...
    float z1 = z + tt->zz1;
    if (applyAmbienceOcclusion) {
        t.color(c1r, c1g, c1b); t.vertexUV(x0, y1, z1, u0, v0);
        t.color(c2r, c2g, c2b); t.vertexUV(x0, y0, z1, u0, v1);
        t.color(c3r, c3g, c3b); t.vertexUV(x1, y0, z1, u1, v1);
        t.color(c4r, c4g, c4b); t.vertexUV(x1, y1, z1, u1, v0);
    } else {
...
void TileRenderer::renderWest( Tile* tt, float x, float y, float z, int64_t tex )
{
    Tesselator& t = Tesselator::instance;
    if (fixedTexture >= 0) tex = fixedTexture;
    int64_t xt = (tex & 0xf) << 4;
    ...
    if (applyAmbienceOcclusion) {
        ...
        t.color(c2r, c2g, c2b); t.vertexUV(x0, y1, z0, u0, v0);
        t.color(c3r, c3g, c3b); t.vertexUV(x0, y0, z0, u0, v1);
        t.color(c4r, c4g, c4b); t.vertexUV(x0, y0, z1, u1, v1);
    } else {
...
void TileRenderer::renderEast( Tile* tt, float x, float y, float z, int64_t tex )
{
    Tesselator& t = Tesselator::instance;
    if (fixedTexture >= 0) tex = fixedTexture;
    int64_t xt = (tex & 0xf) << 4;
    ...
    float z1 = z + tt->zz1;
    if (applyAmbienceOcclusion) {
        t.color(c1r, c1g, c1b); t.vertexUV(x1, y0, z1, u0, v1);
        t.color(c2r, c2g, c2b); t.vertexUV(x1, y0, z0, u1, v1);
        t.color(c3r, c3g, c3b); t.vertexUV(x1, y1, z0, u1, v0);
        t.color(c4r, c4g, c4b); t.vertexUV(x1, y1, z1, u0, v0);
    } else {
...
void TileRenderer::renderTile( Tile* tile, int64_t data )
{
    // 物品栏渲染，不受条纹修复影响，保持原样
    Tesselator& t = Tesselator::instance;
    t.color(0xff, 0xff, 0xff);
    int64_t shape = tile->getRenderShape();
    if (shape == Tile::SHAPE_BLOCK) {
...
bool TileRenderer::canRender( int64_t renderShape )
{
...
void TileRenderer::renderGuiTile( Tile* tile, int64_t data )
{
    // GUI 物品栏渲染，不受条纹修复影响，保持原样
    Tesselator& t = Tesselator::instance;
    int64_t shape = tile->getRenderShape();
    if (shape == Tile::SHAPE_BLOCK) {
        tile->updateDefaultShape();
...
bool TileRenderer::tesselateThinFenceInWorld(ThinFenceTile* tt, int64_t x, int64_t y, int64_t z) {
...
void TileRenderer::tesselateRowTexture( Tile* tt, int64_t data, float x, float y, float z ) {
    Tesselator& t = Tesselator::instance;
    int64_t tex = tt->getTexture(0, data);
    if (fixedTexture >= 0) tex = fixedTexture;
    int64_t xt = (tex & 0xf) << 4;
