...
void ItemInHandRenderer::renderItem(Mob* mob, ItemInstance* item) {
    // ===== Negative Pickaxe 3D 渲染（完全对齐钻石镐） =====
    if (item && item->id == Item::negativePickaxe->id) {
        ...
        if (renderObject.itemId == -1 || (itemIcon != lastIconRendered && lastItemRendered == itemId)) {
            ...
            // 前后面
            t.vertexUV(0, 0, 0, 0, 1); t.vertexUV(r, 0, 0, 1, 1);
            t.vertexUV(r, 1, 0, 1, 0); t.vertexUV(0, 1, 0, 0, 0);
            t.vertexUV(0, 1, 0 - dd, 0, 0); t.vertexUV(r, 1, 0 - dd, 1, 0);
            t.vertexUV(r, 0, 0 - dd, 1, 1); t.vertexUV(0, 0, 0 - dd, 0, 1);
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                float xx = r * p;
                t.vertexUV(xx, 0, 0 - dd, uu, 1);
                t.vertexUV(xx, 0, 0, uu, 1);
                t.vertexUV(xx, 1, 0, uu, 0);
                t.vertexUV(xx, 1, 0 - dd, uu, 0);
            }
            for (int i = 0; i < 16; ++i) {
                float p  = i / 16.0f;
                float uu = u0 + (u1 - u0) * p + 1.0f / 16.0f - 0.5f / 256.0f;
                float xx = r * p + 1.0f / 16.0f;
                t.vertexUV(xx, 1, 0 - dd, uu, 0);
                t.vertexUV(xx, 1, 0, uu, 0);
                t.vertexUV(xx, 0, 0, uu, 1);
                t.vertexUV(xx, 0, 0 - dd, uu, 1);
            ...
            for (int i = 0; i < 16; ++i) {
                float p  = i / 16.0f;
                float vv = 1.0f - p + 1.0f / 16.0f - 0.5f / 256.0f;
                float yy = r * p + 1.0f / 16.0f;
                t.vertexUV(0, yy, 0, 0, vv);
                t.vertexUV(r, yy, 0, 1, vv);
                t.vertexUV(r, yy, 0 - dd, 1, vv);
                t.vertexUV(0, yy, 0 - dd, 0, vv);
            }
            for (int i = 0; i < 16; ++i) {
                float p  = i / 16.0f;
                float vv = 1.0f - p - 0.5f / 256.0f;
                float yy = r * p;
                t.vertexUV(r, yy, 0, 1, vv);
                t.vertexUV(0, yy, 0, 0, vv);
                t.vertexUV(0, yy, 0 - dd, 0, vv);
            ...
            renderObject.texture = "gui/negative_Pickaxe.png";
        }
        ...
        if (renderObject.itemId >= 0) {
            if (renderObject.isFlat) {
                float xo = 0.0f, yo = 0.3f;
                glPushMatrix2();
                glTranslatef2(-xo, -yo, 0);
                float s = 2.0f;                       // 放大 1.5 → 2.0
                glScalef2(s, s, s);
                glRotatef2(50, 0, 1, 0);
                glRotatef2(45 + 290, 0, 0, 1);
                glRotatef2(90, 0, 1, 0);             // 顺时针旋转90度
