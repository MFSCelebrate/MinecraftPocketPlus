...
noiseVals[7] = rls->getForestNoise(pxo * 0.5, pzo * 0.5);
        ...
        noiseVals[0] = els->getPNoise1Value(nX, nY, nZ);
noiseVals[1] = els->getPNoise2Value(nX, nY, nZ);
noiseVals[2] = els->getPNoise3Value(pxo * 17.1103, pyo * 4.277575, pzo * 17.1103);
noiseVals[3] = els->getSNoise1Value(pxo / 16.0, pzo / 16.0);
        ...
        double fY = py * worldScaleY + terrainOffsetY * worldScaleY;
        double fZ = pz * worldScaleZ + terrainOffsetZ * worldScaleZ;
        noiseValsF[0] = (float)els->getPNoise1Value(fX * 1368.824, fY * 684.412, fZ * 1368.824);
noiseValsF[1] = (float)els->getPNoise2Value(fX * 1368.824, fY * 684.412, fZ * 1368.824);
noiseValsF[2] = (float)els->getPNoise3Value(fX * 17.1103, fY * 4.277575, fZ * 17.1103);
noiseValsF[3] = (float)els->getSNoise1Value(fX / 16.0, fZ / 16.0);
...
if (i == 0)
    ...
    // ========== 精度丢失显示（替换原 ln[18] 位置） ==========
    {
        ...
        snprintf(bufBig, sizeof(bufBig), "%.12f", bigStep);
