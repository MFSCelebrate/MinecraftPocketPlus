...
void DungeonFeature::addRoom(Level* level, int xOffs, int zOffs, unsigned char* blocks,
                             float xRoom, float yRoom, float zRoom) {
    addTunnel(level, xOffs, zOffs, blocks, xRoom, yRoom, zRoom,
              1 + random.nextFloat() * 6, 0, 0, -1, -1, 0.5);
}

void DungeonFeature::addTunnel(Level* level, int xOffs, int zOffs, unsigned char* blocks,
