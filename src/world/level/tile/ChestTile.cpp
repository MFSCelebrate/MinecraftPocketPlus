...
void ChestTile::onPlace( Level* level, int64_t x, int64_t y, int64_t z)
{
...
void ChestTile::setPlacedBy( Level* level, int64_t x, int64_t y, int64_t z, Mob* by )
{
	int n = level->getTile(x, y, z - 1); // face = 2
	int s = level->getTile(x, y, z + 1); // face = 3
	int w = level->getTile(x - 1, y, z); // face = 4
	...
	int facing = 0;
	int dir = (Mth::floor64(by->yRot * 4 / (360) + 0.5f)) & 3;
...
void ChestTile::recalcLockDir( Level* level, int64_t x, int64_t y, int64_t z)
{
...
int ChestTile::getTexture( LevelSource* level, int64_t x, int64_t y, int64_t z, int face )
{
...
bool ChestTile::mayPlace( Level* level, int64_t x, int64_t y, int64_t z, unsigned char face )
{
	int chestCount = 0;
...
void ChestTile::neighborChanged( Level* level, int64_t x, int64_t y, int64_t z, int type )
{
...
void ChestTile::onRemove( Level* level, int64_t x, int64_t y, int64_t z)
{
	if (!level->isClientSide) {
		TileEntity* te = level->getTileEntity(x, y, z);
...
bool ChestTile::use( Level* level, int64_t x, int64_t y, int64_t z, Player* player )
{
	TileEntity* te = level->getTileEntity(x, y, z);
...
bool ChestTile::isFullChest( Level* level, int64_t x, int64_t y, int64_t z)
{
